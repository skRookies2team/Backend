...
public class StoryManagementService {
    ...
    public void startGenerationAsync(String storyId) {
        try {
            ...
            if (response.getFileKey() != null && response.getMetadata() != null) {
                ...
                // S3에서 JSON 내용 다운로드
                log.info("Downloading story JSON from S3: {}", finalFileKey);
                storyJson = s3Service.downloadFileContent(finalFileKey);
            }
    ...
    public StoryResultResponseDto getResult(String storyId) {
        ...
        try {
            ...
            // selectedGauges 가져오기 (null 체크 추가)
            List<GaugeDto> selectedGauges;
            List<GaugeDto> contextGauges = fullStory.getContext() != null 
                    ? fullStory.getContext().getSelectedGauges() 
                    : null;
            
            if (contextGauges != null && !contextGauges.isEmpty()) {
                selectedGauges = objectMapper.readValue(
                        storyCreation.getSelectedGaugeIdsJson(),
                        new TypeReference<List<String>>() {}
                ).stream()
                        .map(id -> contextGauges.stream()
                                .filter(g -> g.getId().equals(id))
                                .findFirst()
                                .orElse(null))
                        .filter(g -> g != null) // null 제거
                        .toList();
            } else {
                // context에 selectedGauges가 없으면 빈 리스트 반환
                log.warn("No selectedGauges found in story context for story: {}", storyId);
                selectedGauges = new ArrayList<>();
            }
    ...
    public void startAnalysisFromS3Async(String storyId, String fileKey) {
        try {
            ...
            log.info("Sending request to AI server: /analyze-from-s3, bucket: {}, fileKey: {}", bucketName, fileKey);
            ...
            NovelAnalysisResponseDto response = aiServerWebClient.post()
                    ...
                    .retrieve()
                    .bodyToMono(NovelAnalysisResponseDto.class)
                    .doOnError(error -> log.error("AI server request failed with error: {}", error.getMessage(), error))
                    .block();

            if (response == null) {
                log.error("AI server returned null response");
                throw new RuntimeException("No response from AI server");
            }

            log.info("AI server response received successfully. Summary length: {}, Characters: {}, Gauges: {}",
                    response.getSummary() != null ? response.getSummary().length() : 0,
                    response.getCharacters() != null ? response.getCharacters().size() : 0,
                    response.getGauges() != null ? response.getGauges().size() : 0);
            ...
            log.error("Failed to analyze novel from S3 for story: {}. Error type: {}, Error message: {}", 
                    storyId, e.getClass().getName(), e.getMessage(), e);
            ...
            if (storyCreation != null) {
                ...
                storyCreation.setErrorMessage(e.getClass().getSimpleName() + ": " + e.getMessage());
                storyCreationRepository.save(storyCreation);
            }
        }
    }
